...
void GameControl::recvConfig(char* &buf) {
  if (sscanf(buf, "%d %d %d %d %d %d %d %d %d",  &config.w.na, &config.w.eu, &config.w.ap, &config.j.na, &config.j.eu, &config.j.ap, &config.d.na, &config.d.eu, &config.d.ap)
    != 9) throw Exception("Conversation CONFIG parse fail");
  return;
};

void GameControl::recvDemand(char* &buf) {
  if (sscanf(buf, "%s %d %d %d %d %d %d", sGameDay, &gameHour, &gameMin, &gameSec, &demand.na, &demand.eu, &demand.ap)
	  != 7) throw Exception("Conversation DEMAND parse fail");
  return;
};

void GameControl::recvDist(char* &buf) {
  if (sscanf(buf, "%d %d %d %d %d %d %d %d %d", &dist.w.na, &dist.w.eu, &dist.w.ap, &dist.j.na, &dist.j.eu, &dist.j.ap, &dist.d.na, &dist.d.eu, &dist.d.ap)
	  != 9) throw Exception("Conversation DIST parse fail");
  return;
};

void GameControl::recvProfit(char* &buf) {
  if (sscanf(buf, "%d %d %d %d", &profit, &profitPct, &totalProfit, &totalProfitPct)
    != 4) throw Exception("Conversation PROFIT parse fail");
  return;
};

void GameControl::recvCosts(char* &buf) {
  if (sscanf(buf, "%d %d %d %d", &transProfit, &transWcost, &transJcost, &transDcost)
    != 4) throw Exception("Conversation COSTS parse fail");
  return;
};

void GameControl::serControl(char* &outBuf) {
  snprintf(outBuf, 256, "CONTROL %d %d %d %d %d %d %d %d %d",
