...
void NetSupport::NetworkConversation() {
  ...
  snprintf(sendBuffer, 256, "INIT %s", Team);
  cout << ">> " << sendBuffer << "\n";
  ...
  cout << "      <<  " << recvBuffer << "\n";
  if (sscanf(recvBuffer, "%s", recvCmd)!=1) throw Exception("Reading command");
  if (strcmp(recvCmd,"ACCEPT")) throw Exception("Expected ACCEPT");

  // Send RECD
  snprintf(sendBuffer, 256, "RECD");
//  cout << ">> " << sendBuffer << "\n";
...
//    cout << ">> " << sendBuffer << "\n";
  ...
  snprintf(sendBuffer, 256, "STOP", Team);
  cout << ">> " << sendBuffer << "\n";
